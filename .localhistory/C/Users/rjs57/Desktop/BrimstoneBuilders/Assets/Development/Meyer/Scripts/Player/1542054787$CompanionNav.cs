        ...
        switch ( companion.aggState ){
            ...
            case Companion.AggressionStates.BERZERK:
            {
               ...
               // Debug.Log("character count:" + character.attackers.Count);
                    ...
                    transform.LookAt(character.attackers[currenemy].transform.position);
                    Debug.Log(dist);
                    if (dist < 80)
                    {
                        companion.AnimationClass.Play(AnimationClass.states.AttackTrigger);
                        companion.attachedWeapon.AnimationClass.Play(AnimationClass.weaponstates.EnabledTrigger);
                    }
