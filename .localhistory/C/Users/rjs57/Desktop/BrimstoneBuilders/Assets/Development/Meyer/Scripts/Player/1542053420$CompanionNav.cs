    ...
    protected override void Update( ) {
        switch ( companion.aggState ){
            ...
            case Companion.AggressionStates.BERZERK:
            {
                ...
                currenemy = companion.attackers.Count;
               // Debug.Log("character count:" + character.attackers.Count);
                    Agent.SetDestination(companion.attackers[currenemy].transform.position);
                    float dist = Vector3.Distance(transform.position, companion.attackers[currenemy].transform.position);

                    if (dist < battleDistance)
                    {
                        transform.LookAt(companion.attackers[currenemy].transform.position);
